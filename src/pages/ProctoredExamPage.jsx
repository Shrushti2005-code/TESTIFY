...
export default function ProctoredExamPage() {
  ...
  const submitExam = useCallback(async () => {
    ...
    try {
      ...
      await addDoc(collection(db,"submissions"),{ testName:t.testName, studentEmail:emailRef.current.toLowerCase().trim(), answers:answersRef.current, questions:t.questions||{}, marksPerQuestion:t.marksPerQuestion?.toString()??"0", createdBy:(t.createdBy??"unknown").toLowerCase().trim(), status:"pending", totalMarks:"", maxMarks:"", calculationPath:"", adminFeedback:"", submittedAt:serverTimestamp() });
      setSubmitted(true);
  ...
  if (submitted) return (
    <div style={{ minHeight:"100vh", background:"#F1F5F9", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Poppins',sans-serif" }}>
      <div style={{ background:"#fff", borderRadius:24, padding:"56px 48px", textAlign:"center", border:"1px solid #E2E8F0", maxWidth:420 }}>
        <div style={{ fontSize:56, marginBottom:20 }}>🎉</div>
        <h2 style={{ color:"#1E3A8A", fontSize:26, fontWeight:700, margin:"0 0 12px", fontFamily:"'Georgia',serif" }}>Exam Submitted!</h2>
  ...
  return (
    <div style={s.page}>
      <div style={s.topbar}>
        <div style={s.topbarLeft}>
          ...
          <div>
            ...
            <div style={s.topbarSub}>{studentEmail}</div>
          </div>
        </div>
        <div style={{ ...s.timerBox, background:isUrgent?"#450A0A":"#1E40AF", border:`2px solid ${isUrgent?"#EF4444":"#3B82F6"}` }}>
          <span style={{ fontSize:12, marginRight:5 }}>{isUrgent?"⚠️":"⏱️"}</span>
          <span style={{ ...s.timerText, color:isUrgent?"#FCA5A5":"#BFDBFE" }}>{fmt(timeLeft)}</span>
        </div>
      </div>
      ...
      <div style={{ textAlign:"right", fontSize:11, color:"#64748B", padding:"4px 20px", background:"#fff", borderBottom:"1px solid #E2E8F0", fontWeight:600 }}>
      ...
      <div style={s.layout}>
        ...
        <div style={s.main}>
          ...
          {qEntries.map(([key,text],i) => (
            <div id={`q-${i}`} key={key} onClick={()=>setActiveQ(i)} style={{ ...s.qCard, border:`1.5px solid ${activeQ===i?"#1E3A8A":"#E2E8F0"}`, boxShadow:activeQ===i?"0 0 0 3px rgba(30,58,138,0.08)":"0 1px 4px rgba(0,0,0,0.04)" }}>
              <div style={s.qHeader}>
...
const s = {
  ...
  topbar:       { background:"#1E3A8A", padding:"13px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 },
  topbarLeft:   { display:"flex", alignItems:"center", gap:14 },
  logoBox:      { width:36, height:36, borderRadius:9, background:"#D97706", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:16 },
  topbarTitle:  { color:"#fff", fontWeight:700, fontSize:14, letterSpacing:1 },
  topbarSub:    { color:"#93C5FD", fontSize:11 },
  timerBox:     { display:"flex", alignItems:"center", borderRadius:20, padding:"7px 16px", transition:"all 0.3s" },
  timerText:    { fontWeight:800, fontSize:20, letterSpacing:2 },
  ...
  grid:         { display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6, marginBottom:18 },
  ...
  legend:       { borderTop:"1px solid #E2E8F0", paddingTop:12, marginBottom:18, display:"flex", flexDirection:"column", gap:7 },
  ...
  main:         { flex:1, padding:"22px 28px", maxWidth:720, margin:"0 auto", width:"100%" },
  qCard:        { background:"#fff", borderRadius:14, padding:"20px 22px", marginBottom:16, cursor:"pointer", transition:"all 0.2s" },
  ...
  textarea:     { width:"100%", border:"1.5px solid #E2E8F0", borderRadius:10, padding:"11px 14px", fontFamily:"'Poppins',sans-serif", fontSize:13, color:"#374151", outline:"none", resize:"vertical", background:"#F8FAFC", boxSizing:"border-box" },
