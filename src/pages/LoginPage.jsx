...
export default function LoginPage() {
  ...
  return (
    <div style={s.bg}>
      <div style={s.left}>
        <div style={s.brand}><div style={s.logoBox}>T</div><span style={s.brandName}>TESTIFY</span></div>
        <h1 style={s.headline}>Sign In to<br />Your Account</h1>
        <p style={s.tagline}>Select your role and continue with Google</p>
      </div>
      <div style={s.right}>
        <div style={s.card}>
          ...
          <div style={s.roleRow}>
            {[{key:"student",icon:"🎓",label:"Student"},{key:"admin",icon:"🛡️",label:"Admin"}].map(r => (
              <button key={r.key} onClick={() => setRole(r.key)} style={{
                ...
                <span style={{ fontSize:24, marginBottom:6 }}>{r.icon}</span>
                <span style={{ fontSize:13, fontWeight:600, letterSpacing:1, fontFamily:"'Poppins',sans-serif" }}>{r.label.toUpperCase()}</span>
          ...
          <p style={s.hint}>
            {role === "student" ? "View your assigned tests and results." : "Create tests and evaluate submissions."}
          </p>
...
const s = {
  ...
  headline: { color:"#fff", fontSize:42, fontWeight:700, lineHeight:1.2, margin:"0 0 16px", fontFamily:"'Georgia',serif" },
  tagline:  { color:"#93C5FD", fontSize:15, lineHeight:1.6 },
  right:    { width:480, display:"flex", alignItems:"center", justifyContent:"center", padding:40, background:"#0F172A" },
  card:     { background:"#1E293B", borderRadius:24, padding:"44px 36px", width:"100%", border:"1px solid #334155", animation:"fadeUp 0.4s ease" },
  cardTitle:{ color:"#F1F5F9", fontSize:26, fontWeight:700, margin:"0 0 6px", fontFamily:"'Georgia',serif" },
