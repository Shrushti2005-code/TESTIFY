...
export default function LoginPage() {
  ...
  return (
    <div style={{ ...s.bg, flexDirection: isMobile ? "column" : "row" }}>
      {/* Left panel — hidden on small mobile, shown as top banner */}
      <div style={{
        ...s.left,
        padding: isMobile ? "32px 28px 24px" : "60px 56px",
          ...
          <div style={s.roleRow}>
            {[{ key: "student", icon: "🎓", label: "Student" }, { key: "admin", icon: "🛡️", label: "Admin" }].map(r => (
              <button key={r.key} onClick={() => setRole(r.key)} style={{
                ...
                <span style={{ fontSize: isMobile ? 12 : 13, fontWeight: 600, letterSpacing: 1, fontFamily: "'Poppins',sans-serif" }}>{r.label.toUpperCase()}</span>
