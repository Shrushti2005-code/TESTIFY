...
export default function EvaluationDetailPage() {
  ...
  return (
    <div style={s.page}>
      ...
      {/* Mobile: score summary toggle button */}
      {isMobile && (
        <button onClick={() => setShowSummary(!showSummary)} style={s.summaryToggle}>
          {showSummary ? "▲ Hide Summary" : `▼ Score: ${total}/${maxPossible} (${pct}%)`}
        </button>
      )}

      {/* Mobile: collapsible score + feedback panel */}
      {isMobile && showSummary && (
        <div style={s.mobileSummaryPanel}>
          <ScoreCard total={total} maxPossible={maxPossible} pct={pct} calcString={calcString} />
          <div style={{ marginTop: 16 }}>
            <label style={s.feedbackLabel}>Admin Feedback</label>
            <textarea value={feedback} onChange={e => setFeedback(e.target.value)}
              placeholder="Write feedback for the student..." rows={4} style={s.textarea} />
          </div>
          ...
          <button onClick={submitEvaluation} disabled={submitting} style={{ ...s.submitBtn, opacity: submitting ? 0.7 : 1, marginTop: 12 }}>
      ...
      <div style={{ ...s.layout, flexDirection: isMobile ? "column" : "row" }}>
        {/* Left: questions */}
        <div style={{ ...s.left, padding: isMobile ? "16px" : "28px 24px" }}>
          <div style={{ ...s.infoBar, flexWrap: "wrap", gap: isMobile ? 12 : 24 }}>
          ...
          {qEntries.map(([key, question], i) => (
            <div key={key} style={s.qCard}>
              <div style={{ ...s.qTop, flexWrap: isMobile ? "wrap" : "nowrap", gap: isMobile ? 8 : 12 }}>
                <div style={s.qNum}>Q{i + 1}</div>
                <div style={{ ...s.qText, flex: "1 1 200px" }}>{question}</div>
                <div style={s.markBox}>
          ...
          {/* Mobile: submit button at bottom of questions too */}
          {isMobile && (
            <div style={{ marginTop: 8 }}>
              {msg && <div style={{ ...s.msg, color: msg.startsWith("✅") ? "#166534" : "#991B1B", background: msg.startsWith("✅") ? "#F0FDF4" : "#FEF2F2", border: `1px solid ${msg.startsWith("✅") ? "#86EFAC" : "#FCA5A5"}`, marginBottom: 12 }}>{msg}</div>}
              <button onClick={submitEvaluation} disabled={submitting} style={{ ...s.submitBtn, opacity: submitting ? 0.7 : 1 }}>
                {submitting ? <span style={s.spinner} /> : `Submit Evaluation (${total}/${maxPossible})`}
              </button>
            </div>
          )}
        </div>

        {/* Right: score + feedback — desktop only */}
        {!isMobile && (
          <div style={s.right}>
            <ScoreCard total={total} maxPossible={maxPossible} pct={pct} calcString={calcString} />
...
function ScoreCard({ total, maxPossible, pct, calcString }) {
  return (
    <div style={s.scoreCard}>
      <div style={s.scoreTitle}>Score Summary</div>
      <div style={{ ...s.scoreBig, color: total > maxPossible ? "#EF4444" : "#1E3A8A" }}>
        {total}<span style={{ fontSize: 18, color: "#94A3B8" }}>/{maxPossible}</span>
      </div>
      <div style={s.pctBar}>
        <div style={{ height: "100%", width: `${Math.min(pct, 100)}%`, background: pct >= 80 ? "#16A34A" : pct >= 50 ? "#D97706" : "#EF4444", borderRadius: 6, transition: "width 0.4s" }} />
      </div>
...
function InfoChip({ label, value }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2, minWidth: 0 }}>
      ...
      <span style={{ fontSize: 13, color: "#fff", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 160 }}>{value}</span>
    </div>
  );
}
...
const s = {
  ...
  backBtn:       { background: "none", border: "1px solid #3B82F6", borderRadius: 20, color: "#93C5FD", fontSize: 12, fontWeight: 600, padding: "6px 14px", cursor: "pointer" },
  topbarTitle:   { color: "#fff", fontWeight: 700, fontSize: 14, letterSpacing: 1 },
  adminBadge:    { background: "#1E40AF", color: "#93C5FD", fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 20 },
  summaryToggle: { width: "100%", padding: "12px 16px", background: "#1E293B", color: "#93C5FD", border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer", textAlign: "left" },
  mobileSummaryPanel: { background: "#fff", padding: "16px", borderBottom: "1px solid #E2E8F0" },
  layout:        { display: "flex", minHeight: "calc(100vh - 52px)", alignItems: "flex-start" },
  left:          { flex: 1, overflowY: "auto" },
  ...
  infoBar:       { background: "#1E3A8A", borderRadius: 14, padding: "16px 18px", marginBottom: 16, display: "flex" },
  qCard:         { background: "#fff", borderRadius: 12, padding: "16px 16px", marginBottom: 12, border: "1px solid #E2E8F0", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" },
  qTop:          { display: "flex", alignItems: "flex-start", marginBottom: 12 },
  ...
  qText:         { fontWeight: 600, fontSize: 14, color: "#1E293B", lineHeight: 1.5 },
  markBox:       { display: "flex", alignItems: "center", gap: 6, flexShrink: 0 },
  markInput:     { width: 52, height: 36, border: "2px solid #1E3A8A", borderRadius: 8, textAlign: "center", fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 15, color: "#1E3A8A", outline: "none" },
  markOf:        { fontSize: 12, color: "#94A3B8", fontWeight: 600 },
  answer:        { background: "#F8FAFC", borderRadius: 8, padding: "10px 12px", fontSize: 13, color: "#475569", lineHeight: 1.6 },
  scoreCard:     { background: "#F8FAFC", borderRadius: 14, padding: "18px", border: "1px solid #E2E8F0" },
  scoreTitle:    { fontSize: 11, fontWeight: 700, color: "#64748B", letterSpacing: 1, marginBottom: 12 },
  scoreBig:      { fontSize: 42, fontWeight: 700, lineHeight: 1, marginBottom: 14 },
